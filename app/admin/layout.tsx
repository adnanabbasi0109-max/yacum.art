...
export default function AdminLayout({
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-[#e8e0d0] flex">
      {/* Sidebar */}
      <aside
        className={`${sidebarOpen ? "w-64" : "w-20"} border-r border-[#c9a96e]/20 bg-[#0d0d0d] flex flex-col transition-all duration-300`}
      >
        {/* Logo */}
        <div className="p-6 border-b border-[#c9a96e]/20 flex items-center justify-between">
          <Link href="/admin" className="flex items-center gap-3">
            <Image src="/logo.png" alt="yacum." width={120} height={40} className={sidebarOpen ? "block" : "hidden"} />
            <Image src="/icon.png" alt="y" width={32} height={32} className={sidebarOpen ? "hidden" : "block"} />
          </Link>
          <button
