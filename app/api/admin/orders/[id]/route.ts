...
export async function PUT(
  ...
  try {
    ...
    if (body.fulfillmentStatus) {
      ...
      if (oldStatus !== body.fulfillmentStatus) {
        ...
        order.trackingUpdates.push({
          ...
          message: body.statusMessage || statusMessages[body.fulfillmentStatus] || `Status updated to ${body.fulfillmentStatus}`,
          timestamp: new Date(),
        });
      }
    }
    ...
    if (body.customUpdate) {
      order.trackingUpdates.push({
        status: body.customUpdate.status || order.fulfillmentStatus || 'processing',
